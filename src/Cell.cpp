...
Cell::Cell(int x_, int y_) : x(x_), y(y_), l(0), p(0) {
    ...
    SphereModel.materials[0].shader = cellShader;
}
...
void Cell::drawCell() const {
    ...
    float time = GetTime();
    SetShaderValue(cellShader, timeLoc, &time, SHADER_UNIFORM_FLOAT);
    Vector3 rgb = {(float)cellColor.r / 255.0f, (float)cellColor.g / 255.0f, (float)cellColor.b / 255.0f};
    SetShaderValue(cellShader, baseColorLoc, &rgb, SHADER_UNIFORM_VEC3);
    ...
    for (int i = 0; i < l; i++) {
        ...
        DrawMesh(SphereMesh, SphereModel.materials[0], transform);
    }
}
